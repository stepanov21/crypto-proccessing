...
const Page = () => {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 justify-between px-[50px] text-center text-sm opacity-50 sm:px-6">
      ...
      {data &&
        data.invoice_turnover_period.map((network, key) => {
          return (
            <div
              key={key}
              className="purple-gradient mt-5 grid grid-cols-3 items-center justify-between rounded-[18px] px-[50px] py-7 text-center dark:bg-ourGray dark:bg-none sm:px-6"
            >
              <div className="flex items-center gap-4 text-start">
                <span>{network.currency}</span>
              </div>
              <span>{network.network}</span>
              <span className="roboto text-end">
                {network.total_turnover} USDT
              </span>
            </div>
          );
        })}

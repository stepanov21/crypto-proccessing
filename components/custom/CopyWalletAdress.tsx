...
const CopyWalletAdress = () => {
  const { data, refetch } = useGetMyWallets("erc20");
  ...
  return (
    <>
      ...
      <Button onClick={() => refetch()} className="mb-4 w-full" variant={"default"}>
      ...
      <div className="flex h-[60px] w-full cursor-pointer items-center justify-center gap-4 rounded-full border border-ourGreen text-[18px] font-semibold dark:border-black dark:text-black">
        <CopyToClipboard text={data?.wallet_address} onCopy={() => setCopy(true)}>
